...
| **M0 Vault + repo** | in-progress | This vault and `CLAUDE.md`; past features and decisions recorded; repo renamed `speklone` → `mco`, old address redirects. |
| **M1 Port** | planned | Vite + TypeScript + Svelte project; analysis, parsing, verdict and stems code moved into `src/core` unchanged apart from types; real module workers; today's UI rebuilt as "Analyze a file"; known bugs fixed (analysis race, lost stem Cancel, stuck busy bar, duplicate play loops); GitHub Actions deploy; parity tests against the old page. |
