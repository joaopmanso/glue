...
export async function handle(req: Request, env: Env, deps: Deps): Promise<Response> {
  ...
  try {
    ...
    if (m === 'POST' && path === '/v1/pairing') {
      ...
      // The browser that makes the code: the GLUE Home that claims it becomes its companion (ADR 0045).
...
async function claim(env: Env, b: Record<string, unknown>, now: number, ip: string) {
  ...
  const pc = await env.DB.prepare('SELECT user_id, expires_at, used_at, device_id FROM pairing_codes WHERE hash = ?').bind(h).first<{ user_id: string; expires_at: number; used_at: number | null; device_id: string | null }>();
  ...
  const of = pc.device_id ? await env.DB.prepare("SELECT id, name FROM devices WHERE id = ? AND user_id = ? AND kind = 'browser' AND revoked_at IS NULL").bind(pc.device_id, pc.user_id).first<{ id: string; name: string }>() : null;
  // Connecting again (a new code): the GLUE Home's previous device goes, if it proves it's its own.
  const prev = b.replaces as { deviceId?: unknown; token?: unknown } | undefined;
  if (prev && typeof prev.deviceId === 'string' && typeof prev.token === 'string') {
    const own = await env.DB.prepare("SELECT 1 FROM credentials WHERE hash = ? AND device_id = ? AND user_id = ? AND kind = 'device'").bind(await sha256(prev.token), prev.deviceId, pc.user_id).first();
    if (own) await revokeDevice(env, pc.user_id, prev.deviceId, now);
  }
...
async function newHome(env: Env, userId: string, b: Record<string, unknown>, now: number, of: { id: string; name: string } | null = null) {
  const id = randomId(), token = randomToken(), name = str(b.name, 60) || of?.name || 'GLUE Home';
  await env.DB.batch([
    env.DB.prepare('INSERT INTO devices (id, user_id, kind, name, platform, public_key, created_at, last_seen, companion_of) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id, userId, 'home', name, str(b.platform, 60) || null, str(b.publicKey, 2000) || null, now, now, of?.id ?? null),
  ...
  return { deviceId: id, token, name, user, companionOf: of ? { id: of.id, name: of.name } : null };
}
/** Revoke a device: it can't refresh or reconnect, and is dropped from the signaling room at once.
    Its synced collections go too (ADR 0040). */
async function revokeDevice(env: Env, userId: string, id: string, now: number) {
  await env.DB.batch([
    env.DB.prepare('UPDATE devices SET revoked_at = ? WHERE id = ? AND user_id = ?').bind(now, id, userId),
    env.DB.prepare('DELETE FROM credentials WHERE device_id = ?').bind(id),
    env.DB.prepare('DELETE FROM sync_files WHERE device_id = ?').bind(id),
    env.DB.prepare('DELETE FROM sync_profiles WHERE device_id = ?').bind(id),
    env.DB.prepare('DELETE FROM sync_links WHERE device_id = ?').bind(id),
    env.DB.prepare('DELETE FROM sync_ops WHERE device_id = ?').bind(id),
  ]);
